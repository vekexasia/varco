...
This page covers consumer-to-Authority messages, which travel end-to-end encrypted through the bridge. The plaintext Authority-to-bridge handshake (`challenge`/`auth`/`ready`, including the `proto` version field) is documented in [Bridge endpoints](../bridge-endpoints/).
...
| `authenticate` | `consumer_pk`, `nonce`, `signature` | `authenticated` | Active grant for consumer key. The signature covers `varco-authenticate-v1\0` + `nonce` + `\0` + the session channel binding, so it cannot be replayed in another encrypted session. |
