...
- Consumers can choose how the upgrade happens with `connectionStrategy`: `webrtc-first` (default, await the upgrade then fall back to relay), `webrtc-only` (require P2P, fail otherwise), or `optimistic` (use relay immediately and upgrade in the background).
- Consumers can disable WebRTC entirely with the legacy `webrtc: false` flag (relay-only).
