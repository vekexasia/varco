...
- `@varco/client`: published to npm from `packages/client` when `NPM_TOKEN` is configured.
...
The GitHub release workflow publishes with npm provenance when `NPM_TOKEN` is available. Manual publishing is:
