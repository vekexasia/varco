...
:::caution[After updates]
After every Varco update, restart Home Assistant and hard-refresh `/varco`. Home Assistant keeps the old Python integration and panel JavaScript in memory until restart/browser refresh.
:::
...
## Dashboard export and build

The panel can harvest a Lovelace dashboard or view into a selected entity manifest. Use **Download agent brief zip** to export `brief.md` and `manifest.json` for a coding agent, or **Build share link** to mint a read-only multi-entity claim link from the same selection.
