...
- Confirm WebRTC works: do not disable the upgrade (`webrtc: false`), and check Home Assistant logs for `aiortc` warnings.
...
- Confirm the consumer works on relay only (`webrtc: false`, or omit `connectionStrategy` so it falls back to relay).
