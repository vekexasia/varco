...
How the bridge URL is stored depends on how the integration was set up:

- **YAML setup** (`varco:` in `configuration.yaml`): edit `bridge_ws_url` and restart Home Assistant.
- **UI setup** (Settings -> Devices & services): the URL is pinned in the config entry at setup time. There is currently no options or reconfigure flow to change it. Removing and re-adding the integration changes the URL but generates a new Authority keypair, which invalidates every existing grant and changes your Authority ID, so consumers must re-pair.

If you need to keep the Authority keypair, edit `bridge_ws_url` in the entry data inside `config/.storage/core.config_entries` while Home Assistant is stopped, then start it again. Back up the file first; `.storage` files are not a supported editing surface.
