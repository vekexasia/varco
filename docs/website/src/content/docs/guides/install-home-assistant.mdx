...
:::caution[Updating Varco]
After every HACS or manual Varco update, restart Home Assistant and hard-refresh `/varco`. Without restart, Home Assistant continues running the previously loaded Python integration; without hard refresh, the browser may keep the old panel bundle.
:::
