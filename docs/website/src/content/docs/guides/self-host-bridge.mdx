...
For a real deployment, keep the container on localhost and put a TLS reverse proxy in front of it. Public Home Assistant and consumer URLs should use `wss://`, for example:
...
With Cloudflare in front, use a proxied DNS record for `bridge.example.com` pointing at the server and let Cloudflare terminate public TLS. If you terminate TLS locally instead, use your normal certificate automation and expose nginx on `443`.
