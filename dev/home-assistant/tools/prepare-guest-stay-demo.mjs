...
const outputPath = process.argv[2] || 'examples/guest-stay-showcase/src/generated-demo-grant.ts';
...
const { createGuestStayManifest, READ_ENTITIES, HOUSE_ENTITIES } = await import('../../../examples/guest-stay-showcase/dist/config.js');
...
try {
  ...
  client = createVarcoClient({
    authorityId,
    bridgeUrl,
    manifest: createGuestStayManifest(),
    webrtc: false,
    storage,
  });
  ...
  await client.callService('switch', 'turn_on', { entity_id: HOUSE_ENTITIES.coffeeMachine });
  await client.callService('switch', 'turn_off', { entity_id: HOUSE_ENTITIES.coffeeMachine });
  ...
  console.log(`Prepared guest stay demo grant ${grant.grant_id || access.request_id} for Authority ${authorityId}`);
} finally {
  await client?.close?.();
  admin.close?.();
}
