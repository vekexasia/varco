...
const grantedHistoryEntitySet = new Set<string>(HISTORY_ENTITIES);
const deniedHistoryEntitySet = new Set<string>(HISTORY_LINK_ENTITIES.filter((entity) => !grantedHistoryEntitySet.has(entity)));
...
function node(label: string, value: string, unit: string, entity: string, tone = ""): string {
  ...
  return `<div class="node${clickable ? " clickable" : ""}"${tone ? ` data-tone="${tone}"` : ""}${clickable ? ` data-entity="${entity}"` : ""}><div class="nicon">${esc(label)}</div><div class="nval">${esc(value)}<i>${esc(unit)}</i></div><div class="nlbl">${esc(entityLabel(entity))}</div>${historyScopeBadge(entity)}</div>`;
}
...
function historyScopeBadge(entity: string): string {
  return deniedHistoryEntitySet.has(entity) ? '<div class="scope-badge">history not granted</div>' : "";
}
...
async function openHistoryChart(entityId: string): Promise<void> {
  ...
  try {
    ...
    const body = modal.querySelector(".modal-body") as HTMLElement | null;
    if (deniedHistoryEntitySet.has(entityId)) {
      const title = modal.querySelector(".modal-title") as HTMLElement | null;
      if (title) title.textContent = "Access denied by grant";
      if (body) body.innerHTML = `<div class="status error">History not granted</div><p class="modal-note"><code>${esc(entityId)}</code> is readable live, but historical data was intentionally left out of this demo grant. The frontend can ask anyway; Home Assistant still enforces the stored permission.</p>`;
      return;
    }
